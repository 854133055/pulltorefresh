...
//						try {
                            //TODO uelog	UELogUtils.UElog(this.getContext().getClass().getSimpleName(),  QunarApp.getContext().getResources().getResourceEntryName(this.getId()));
//						}catch(Exception e){}
    ...
    /**
     * 手动触发刷新请求（只触发mOnRefreshListener，不触发mOnRefreshListener2）
     */
    public final void requestToRefresh() {
        if (!isRefreshing()) {
            setRefreshing(true);
            if (null != mOnRefreshListener) {
                mOnRefreshListener.onRefresh(this);
    ...
    /**
     * Called by {@link #onRestoreInstanceState(android.os.Parcelable)} so that derivative classes can handle their saved instance
     * state.
     *
	...
	protected void onReset() {
		smoothScrollTo(0, SMOOTH_SCROLL_DURATION_MS, 0, new OnSmoothScrollFinishedListener() {
		...
		//在向上滑之前，不能重置，否则会出现最后一闪的错误状态，放在下拉刷新的时候重置
    }
    ...
    protected void setHeaderScroll(final int value) {
        ...
        /**
         * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer
         * Views as they change often, which would negate any HW layer performance boost.
         */
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.pub_fw_PullToRefresh, defStyle, 0);

        if (a.hasValue(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrMode, 0));
        }

        if (a.hasValue(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.pub_fw_PullToRefresh_pub_fw_ptrAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrRefreshableViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrOverScroll, true);
        }

        if (a.hasValue(R.styleable.pub_fw_PullToRefresh_pub_fw_ptrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.pub_fw_PullToRefresh_pub_fw_ptrScrollingWhileRefreshingEnabled, false);
        }
    ...
    public static enum AnimationStyle {
        ...
        FLIP,
        /**
         * This is added by zitian.zhang, aiming at implementing a camel running.
         */
        ANIM_DRAWABLE, ;
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            ...
            case FLIP:
				//在更改公共的加载时，未了避免大量更改布局，所以选择调整名字预制的对应关系
				return new AnimationDrawableLoadingLayout(context, mode, scrollDirection, attrs);
            case ANIM_DRAWABLE:
